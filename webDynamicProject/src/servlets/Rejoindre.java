...
public class Rejoindre extends HttpServlet {
	...
	private static final int DELAIS = 9000;
	@EJB
	GestionParties gp;
...
//			request.getRequestDispatcher("/menuverif.html").forward(request, response);
			...
			// gp.rejoindreLaPartie(pseudo); // Utiliser la m�thode avec
			// dernier id plutot que de tout parcourir pour trouver la bonne
			// partie
			// System.out.println("Bienvenue "+pseudo);
			// request.getRequestDispatcher("jeu.html").forward(request,
			// response);
			// getServletContext().getNamedDispatcher("jeu.html").forward(request,
			// response);
			// response.sendRedirect("jeu.html");
			// getServletContext().getNamedDispatcher("jeumanager.html").forward(request,
			// response);
		}
	}
